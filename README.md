...
## BDD high level overview from Gáspár Nagy.
Story (user story) is a goal that we have to achieve. Rule (acceptance criteria) sets scope to Story. Rule produces
System (production code). Coded test (test & automation code) verifies System. Example explains Rule. Example produces
Coded test. Scenario illustrates Rule. Scenario verifies System. Step definition (automation code) defines Scenario.
Rule by discovery produces Example. Example by formulation produces Scenario.
...
Libraries are shareable across libraries and applications. They can be imported from `@requirements/mylib`.
